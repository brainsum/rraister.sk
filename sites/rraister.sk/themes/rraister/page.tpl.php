...
?> 
  <div id="page-outer" class="<?php print ($is_front ? 'frontpage' : 'subpage')?>">
    <div id="page-inner" class="<?php if( isset($node->nid) ){ print 'node-page node-page-'.$node->type.' node-page-id-'.$node->nid; } ?>">
      ...
      <div id="page_container">
        ...
        <?php endif; ?>
          ...
          <?php if ($title and !$is_front ): ?>
            <?php
            $extra_title_class='';
            if( strpos( $title, '"sub"' )> 0  ){
              $extra_title_class = 'sub';
            }
            ?>
            <h1<?php print $tabs ? ' class="with-tabs '.$extra_title_class.'"' : '' ?>><?php print $title ?></h1>
          <?php endif; ?>
      ...
      <div id="footer">
        <div class="copyright">© 1997 - 2007 RRA ISTER. Všetky práva vyhradené.</div>
